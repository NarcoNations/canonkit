...
- The intended package identity is `@narconations/canonkit`; the CLI command remains `canonkit`. Registry ownership is not assumed until the scope is created and verified.
...
| TM-004 | Malicious repository contributor | CanonKit scans an untrusted large branch in local or CI context | Add up to 10,000 near-1 MiB documents so aggregate retained input consumes excessive memory/time | CI or workstation denial of service | Availability, CI resources | Per-file 1 MiB and discovery 10,000-document limits; sequential bounded parsing (`src/metadata/frontmatter.ts`, `src/discovery/repository.ts`) | No aggregate repository-byte budget exists before normalized bodies are retained | Add a default and hard aggregate scan-byte budget with atomic failure before alpha; test near-limit repositories | Track run duration and peak memory; fail jobs on budget diagnostic | Medium | High | high |
...
| TM-006 | Compromised maintainer, dependency, GitHub Action, or publisher credential | A public release pipeline exists with write authority | Alter source/build or publish a malicious version under the intended package identity | Arbitrary code executes for downstream installers/users | Package artifact, release authority, downstream systems | Lockfile; current CI has `contents: read`, no secrets, and no publish step (`package-lock.json`, `.github/workflows/ci.yml`) | No protected trusted-publishing workflow, npm scope, provenance, approval environment, or recovery policy exists | Use npm trusted publishing/OIDC, provenance, protected environment approval, least permissions, immutable action SHAs, account MFA, exact tarball verification, and no routine local publish | npm/GitHub audit logs, provenance verification, release diff and install smoke test | Low before release; Medium after | High | high |
| TM-007 | Package-name squatter or user confusion | User follows ambiguous installation guidance | Install the unrelated unscoped `canonkit` package instead of this project | Execution of unintended third-party code and loss of project identity | Users, package identity | Repository URL is explicit; current package remains `private: true` and unpublished (`package.json`) | Unscoped name is occupied; proposed scope is not yet owned or configured | Use only `@narconations/canonkit`; reserve and verify the npm scope; change metadata and docs together; never recommend unscoped installation | Registry ownership check and clean-environment install assertion | High if guidance is ambiguous | High | high |
...
| `src/model/collection.ts` | Reads and retains repository bodies and is the right place for an aggregate byte budget | TM-004 |
