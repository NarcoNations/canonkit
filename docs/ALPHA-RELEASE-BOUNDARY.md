...
Stage 5.1 defines the intended first public package as:

- **Package:** `@narconations/canonkit`
- **CLI binary:** `canonkit`
- **Initial version:** `0.1.0-alpha.0`
- **Access:** public scoped package
- **Source:** the `NarcoNations/canonkit` GitHub repository
- **Runtime:** Node.js 22 or newer
- **Licence:** MIT

This is a release contract, not a publication. Stage 5.1 does not create an npm organisation, authenticate a publisher, change `package.json`, tag Git, or publish an artifact.

The unscoped `canonkit` name is occupied by an unrelated package. CanonKit documentation and automation must never instruct users to install that unscoped name. A registry lookup on 2026-08-21 returned no public package for `@narconations/canonkit`, but that does not prove ownership or reserve the name.
...
4. The same verified tarball is published publicly as `@narconations/canonkit@0.1.0-alpha.0` with provenance.
...
- npm scope ownership and recovery controlled by the maintainer
